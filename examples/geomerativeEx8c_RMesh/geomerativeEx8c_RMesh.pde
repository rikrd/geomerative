...
void draw()
{
    ...
    for ( int i = 0; i < mesh.strips.length; i++ )
    {
        ...
        for ( int ii = 2; ii < pnts.length; ii++ )  // "triangles", dreiecke durchlaufen
        {
            ...
            pushMatrix();
                translate( xCntr, yCntr );         // ins zentrum
                rotate( (frameCount+ii) / 300.0 ); // ums zentrum rotieren
                beginShape();
                    vertex( pnts[ii-2].x-xCntr, pnts[ii-2].y-yCntr );
                    vertex( pnts[ii-1].x-xCntr, pnts[ii-1].y-yCntr );
                    vertex( pnts[ii].x-xCntr,   pnts[ii].y-yCntr );
                endShape(CLOSE);
            popMatrix();
            
            fill( 60 );
            
            ellipse( xCntr, yCntr, 5, 5 );
        }
    }
}
